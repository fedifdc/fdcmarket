...
if ( ! function_exists( 'mycred_woo_payout_rewards' ) ) :
	function mycred_woo_payout_rewards( $order_id ) {
		...
		foreach ( $types as $point_type => $point_type_label ) {
			...
			add_filter('mycred_all_references', 'mycred_purchase_product_reference');

			function mycred_purchase_product_reference($references) {
    		$references['reward'] = __('reward for store purchase', 'mycredpartwoo');
    		return $references;
			}
			*/
			
			/**
			* Add Purchase Product References
			* by Fedi
 			* @version 1.0
 				*/
			add_filter( 'mycred_all_references', 'mycred_purchase_product_reference' );
			...
			$references['reward'] = 'reward for store purchase';

				return $references;

				}

			/** Let others play with the reference and log entry (backup)
			...
			$reference = apply_filters( 'mycred_all_references', 'reward', $order_id, $point_type );
			$log       = apply_filters( 'mycred_woo_reward_log',       '%plural% reward for store purchase', $order_id, $point_type );
